...
import deleteIconSrc from '../../assets/svg/delete.svg';
...
class Categories extends React.Component {
  ...
  componentDidMount() {
    this.container = d3.select(this.refCategories.current);
    this.deleteIcon = this.container.append('svg:image')
  ...
  calculateData() {
    ...
    this.categories = _.map(this.props.categories, category => {
      ...
      .filter( expense => d3.timeWeek.floor(expense.date).getTime() === this.props.selectedWeek.getTime())
      .each(expense => {
    ...
    if(this.props.categoryBeingAdded) {
      this.categories.push({...this.props.categoryBeingAdded, fx: this.props.width / 2, fy: 100, radius})
    }
  }
